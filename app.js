...
function setDefaultPaymentMethods() {
  State.paymentMethods = [
    { name: '現金', type: '口座', initialBalance: 0, order: 1, active: true, owner: '共有' },
    { name: 'クレカ', type: 'クレジット', initialBalance: 0, order: 2, active: true, owner: '共有' },
    { name: 'PayPay', type: '電子マネー', initialBalance: 0, order: 3, active: true, owner: '共有' },
    { name: '銀行口座', type: '口座', initialBalance: 0, order: 4, active: true, owner: '共有' },
    { name: '貯金口座', type: '貯金', initialBalance: 0, order: 5, active: true, owner: '共有' },
  ];
}
...
// ===== 残高タブ切り替え =====
function switchBalanceTab(tab, btn) {
  document.querySelectorAll('.balance-tab').forEach(b => b.classList.remove('active'));
  document.querySelectorAll('.bal-panel').forEach(p => p.classList.remove('active'));
  if (btn) btn.classList.add('active');
  const panel = document.getElementById('bal-panel-' + tab);
...
function refreshSummary() {
  ...
  // 残高：個人別＋合計
  const yumika  = calcBalanceByOwner('ゆみか', false);
  const kanta   = calcBalanceByOwner('かんた', false);
  const total   = calcBalanceByOwner('全員',   false);
  const savingsY = calcBalanceByOwner('ゆみか', true);
  const savingsK = calcBalanceByOwner('かんた', true);
  const savingsT = calcBalanceByOwner('全員',   true);

  setText('balance-yumika',  '¥' + fmt(yumika));
  setText('balance-kanta',   '¥' + fmt(kanta));
  setText('balance-total',   '¥' + fmt(total));
  setText('savings-yumika',  '¥' + fmt(savingsY));
  setText('savings-kanta',   '¥' + fmt(savingsK));
  setText('savings-total',   '¥' + fmt(savingsT));
}

// owner: 'ゆみか' | 'かんた' | '全員'、isSavings: 貯金口座のみか
function calcBalanceByOwner(owner, isSavings) {
  const pms = State.paymentMethods.filter(pm => {
    const typeMatch = isSavings ? pm.type === '貯金' : pm.type !== '貯金';
    const ownerMatch = owner === '全員' || !pm.owner || pm.owner === owner || pm.owner === '共有';
    return typeMatch && ownerMatch;
  });
  const pmNames = new Set(pms.map(p => p.name));
  let balance = pms.reduce((s, p) => s + (p.initialBalance || 0), 0);

  State.transactions.forEach(t => {
    const pmFrom = State.paymentMethods.find(p => p.name === t.payment);
    const pmTrFrom = State.paymentMethods.find(p => p.name === t.transferFrom);
    const pmTrTo   = State.paymentMethods.find(p => p.name === t.transferTo);

    if (t.type === TYPE.INCOME && pmFrom && pmNames.has(pmFrom.name)) {
      balance += t.amount;
    } else if (t.type === TYPE.EXPENSE && pmFrom && pmNames.has(pmFrom.name)) {
      balance -= t.amount;
    } else if (t.type === TYPE.TRANSFER) {
      if (pmTrFrom && pmNames.has(pmTrFrom.name)) balance -= t.amount;
      if (pmTrTo   && pmNames.has(pmTrTo.name))   balance += t.amount;
    }
  });
  return balance;
}
...
function calcAccountBalance(isSavings) {
  return calcBalanceByOwner('全員', isSavings);
}
    ...
    .map(p => `
      <div class="manage-item">
        <div>
          ...
          <div class="manage-item-type">${esc(p.type)} · ${esc(p.owner || '共有')} · 初期残高 ¥${fmt(p.initialBalance || 0)}</div>
        </div>
        <div class="manage-item-actions">
...
function openAddPaymentMethod() {
  ...
  document.getElementById('pm-owner').value = '共有';
...
function editPaymentMethod(name) {
  ...
  document.getElementById('pm-owner').value = pm.owner || '共有';
...
async function savePaymentMethod() {
  ...
  const owner = document.getElementById('pm-owner').value || '共有';
  const originalName = document.getElementById('pm-original-name').value;
  if (!name) { showToast('名前を入力してください', 'error'); return; }

  try {
    const data = { name, type, initialBalance, owner, originalName: originalName || name };
    if (originalName) {
      ...
      if (idx !== -1) State.paymentMethods[idx] = { ...State.paymentMethods[idx], name, type, initialBalance, owner };
      showToast('更新しました', 'success');
    } else {
      ...
      State.paymentMethods.push({ name, type, initialBalance, owner, order: maxOrder + 1, active: true });
      showToast('追加しました', 'success');
    }
    saveToCache();
    closeModal();
